                          ...
                          void *user, void *in, size_t len)
{
    ...
    case LWS_CALLBACK_RECEIVE:
        ...
        if (strcmp(msg_buf, "toggle_yolo") == 0) {
            ctx->cmd_req.yolo_req = !ctx->cmd_req.yolo_req;
        } else if (strcmp(msg_buf, "toggle_edge") == 0) {
            ctx->cmd_req.edge_req = !ctx->cmd_req.edge_req;
        } else if (strcmp(msg_buf, "colormap_next") == 0) {
            if (ctx->cmd_req.colormap_ctrl == COLORMAP_GRAYS) {
                ctx->cmd_req.colormap_ctrl = COLORMAP_CLASSIC;
            } else {
                ctx->cmd_req.colormap_ctrl += 1;
            }
        } else if (strncmp(msg_buf, "sync-req=", 9) == 0) {
            int req_val = atoi(msg_buf + 9);
...
static struct lws_protocols protocols[] = {
    {
        ...
        .per_session_data_size = sizeof(thread_context_t*),
        .rx_buffer_size = 0,
    },
    { NULL, NULL, 0, 0 }
};
